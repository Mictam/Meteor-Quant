...
interface MarketLMCapabilities {
...
function initialIndicatorState(capabilities: MarketLMCapabilities): Record<string, IndicatorState> {
...
export default function MarketLMTab({
  dataset,
  onModelRegistered,
}: {
  ...
  const [capabilities, setCapabilities] = useState<MarketLMCapabilities | null>(null);
  const [runs, setRuns] = useState<MarketLMRun[]>([]);
  const [models, setModels] = useState<RegisteredModel[]>([]);
  const [selectedRunId, setSelectedRunId] = useState('');
  const [selectedRun, setSelectedRun] = useState<MarketLMRun | null>(null);
  const [indicatorState, setIndicatorState] = useState<Record<string, IndicatorState>>({});
  const [name, setName] = useState('btc-marketlm-custom');
  const [timeframeSeconds, setTimeframeSeconds] = useState(1);
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [horizons, setHorizons] = useState('5,15,30,60,300');
  const [patchSize, setPatchSize] = useState(8);
  const [contextPatches, setContextPatches] = useState(384);
  const [trainFraction, setTrainFraction] = useState(0.8);
  const [validationFraction, setValidationFraction] = useState(0.1);
  const [costThresholdBps, setCostThresholdBps] = useState(2);
  ...
  const [minLearningRate, setMinLearningRate] = useState(0.00002);
  ...
  const refresh = useCallback(async () => {
    const response = await api<MarketLMResponse>('/api/marketlm/runs');
  ...
  useEffect(() => {
    Promise.all([
      api<MarketLMCapabilities>('/api/marketlm/capabilities'),
      api<MarketLMResponse>('/api/marketlm/runs'),
      ...
      const defaultPreset = capabilityResponse.model_presets.quality_4060ti;
  ...
  useEffect(() => {
    ...
    api<MarketLMRun>(`/api/marketlm/runs/${selectedRunId}`)
      .then(setSelectedRun)
      .catch((cause: unknown) => setError(cause instanceof Error ? cause.message : String(cause)));
  }, [selectedRunId, runs]);
  ...
  const requestPayload = (prepareOnly: boolean) => ({
    ...
    training: {
      ...
      prefetch_factor: 2,
      autoregressive_loss_weight: 0.5,
      forecast_loss_weight: 2,
  ...
  const startRun = (prepareOnly: boolean) => runOperation(
    prepareOnly ? 'Preparing MarketLM tensors…' : 'Starting MarketLM training…',
    async () => {
      const run = await api<MarketLMRun>('/api/marketlm/runs', {
        method: 'POST',
  ...
  const stopRun = (runId: string) => runOperation('Stopping MarketLM worker…', async () => {
    await api(`/api/marketlm/runs/${runId}/stop`, { method: 'POST' });
    await refresh();
  });

  const registerRun = (run: MarketLMRun) => runOperation('Registering model as indicator…', async () => {
    await api(`/api/marketlm/runs/${run.run_id}/register`, {
      method: 'POST',
      body: JSON.stringify({ display_name: run.name, checkpoint: 'best' }),
    });
    await Promise.all([refresh(), onModelRegistered()]);
  });

  const unregisterModel = (modelId: string) => runOperation('Unregistering model…', async () => {
    await api(`/api/marketlm/models/${modelId}`, { method: 'DELETE' });
    await Promise.all([refresh(), onModelRegistered()]);
  });
  ...
  return (
    <main className="marketlm-workspace">
      <section className="marketlm-hero">
        <div>
          <div className="eyebrow">CUSTOM FEATURES · CAUSAL LABELS · RESUMABLE TRAINING</div>
          <h2>Train MarketLM</h2>
          <p>Build a causal transformer from the existing Binance CSV/Parquet history, selected indicators, and configurable forecast horizons. Completed checkpoints register as reusable chart indicators and threshold strategies.</p>
        </div>
      ...
      {error && <div className="alert error"><strong>MarketLM action failed</strong><span>{error}</span></div>}
      ...
      <div className="marketlm-grid">
        ...
        <section className="marketlm-runs">
          ...
          <div className="run-list">
            {runs.length === 0 && <div className="empty-inline">No MarketLM runs yet.</div>}
            {runs.map((run) => (
