...
from meteor_quant.markethybrid.jobs import update_status_file, utc_now
from meteor_quant.markethybrid.schemas import MarketHybridRunRequest
from meteor_quant.markethybrid.training import train_markethybrid
...
def run_worker(run_dir: Path) -> None:
    ...
    request = MarketHybridRunRequest.model_validate(spec["request"])
    ...
    def preparation_progress(progress: float, message: str) -> None:
        update(
            ...
            message=message.replace("MarketLM", "MarketHybrid"),
        )
    ...
    try:
        update(
            state="preparing",
            progress=0.0,
            message="preparing MarketHybrid data",
            pid=os.getpid(),
        )
        catalog = DatasetCatalog(Path(spec["data_dir"]))
        data_request = MarketLMRunRequest(
            name=request.name,
            data=request.data,
            prepare_only=True,
        )
        ...
        if request.prepare_only:
            terminal = {
                "state": "completed",
                "progress": 1.0,
                "message": "MarketHybrid data preparation completed",
                "prepared_dir": str(prepared),
            }
            update(**terminal)
            write_result(**terminal)
            return
        final_path = train_markethybrid(
            request,
            prepared,
            run_dir,
        ...
        write_result(
            ...
            message="MarketHybrid training completed",
            prepared_dir=str(prepared),
        ...
        if current.get("state") not in {"stopped", "completed"}:
            update(state="stopped", message="MarketHybrid worker stopped")
        write_result(state="stopped", message="MarketHybrid worker stopped")
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: python -m meteor_quant.markethybrid.worker RUN_DIR")
