...
from meteor_quant.marketlm.features import indicator_capabilities
...
class MarketHybridJobManager:
    def __init__(self, project_root: Path, data_dir: Path) -> None:
        self.project_root = project_root
        self.data_dir = data_dir
        self.root = data_dir / "markethybrid"
        self.runs_dir = self.root / "runs"
        self.prepared_dir = data_dir / "marketlm" / "prepared"
    ...
    def capabilities(self) -> dict[str, Any]:
        ...
        return {
            ...
            "default_indicators": [
                ...
                for item in MarketHybridRunRequest().data.indicators
            ],
            "default_config": MarketHybridRunRequest().model_dump(mode="json"),
    ...
    def start(self, request: MarketHybridRunRequest) -> dict[str, Any]:
        if not self.torch_available and not request.prepare_only:
            raise RuntimeError(
                "PyTorch is not installed. Run .\\setup-markethybrid.ps1 or install the markethybrid extra."
            )
        active = [item for item in self.list_runs() if item.get("state") in _ACTIVE_STATES]
        if active:
            raise RuntimeError(
                f"MarketHybrid job {active[0]['run_id']} is already {active[0]['state']}; "
        ...
        process = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "meteor_quant.markethybrid.worker",
                str(run_dir),
            ],
    ...
    def get(self, run_id: str) -> dict[str, Any]:
        ...
        if not path.exists():
            raise KeyError("MarketHybrid run not found")
    ...
    def register(
        ...
        if status.get("state") != "completed":
            raise ValueError("only completed MarketHybrid runs can be registered")
        ...
        if not isinstance(prepared_value, str) or not Path(prepared_value).exists():
            raise ValueError("prepared MarketHybrid tensors are missing for this run")
        spec = read_json(run_dir / "spec.json")
        run_request = MarketHybridRunRequest.model_validate(spec["request"])
        ...
        payload = {
            ...
            or "Custom MarketHybrid forecast indicator trained in Meteor Quant.",
    ...
    def unregister(self, model_id: str) -> None:
        ...
        if not path.exists():
            raise KeyError("registered MarketHybrid model not found")
        path.unlink()
    ...
    def _run_dir(self, run_id: str) -> Path:
        if not run_id or any(
            ...
            raise KeyError("invalid MarketHybrid run id")
